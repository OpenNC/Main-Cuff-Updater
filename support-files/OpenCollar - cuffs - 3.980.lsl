...
default
{
            ...
            else
            {
                ...
                else 
                {
                    ...
                    else
                    {
                        ...
                        if ((str1 =="Show Collar") || (str1 =="Show All") || (str =="show") ||  (str1 =="☐ Collar"))
                        {
                            llRegionSayTo(wearer,CUFF_CHANNEL,(string)id + ":cshow");
                            hide = FALSE;
                        }
                        else if ((str1 =="Hide Collar") || (str1 =="Hide All") || (str =="hide") ||  (str1 =="☒ Collar"))
                        {
                        ...
                        else if ((str1 ==" LOCK") && (num == DIALOG_RESPONSE) && (wait = FALSE))
                        {
                              llMessageLinked(LINK_SET, LM_SETTING_SAVE, "C_lock=1", NULL_KEY);
                                wait = TRUE;
                                llSetTimerEvent(0.5);//stops lock bouncing due to multi messages from collar
                        }
        ...
        if (id==g_kDialogID)
            {
                ...
                {//lets grab the saved settings so we can forward them on
                    ...
                    llSetTimerEvent(0.1);//to slow down blocks of messages so we are not over run
    ...
    timer()
    {//this is to stop some double syncing actions due to how collar sends messages
        wait = FALSE;
        llSetTimerEvent(0);
    }
}